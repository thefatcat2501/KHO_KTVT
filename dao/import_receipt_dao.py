"""dao/import_receipt_dao.py - CRUD phiếu nhập kho (đọc dữ liệu / lịch sử).
Việc TẠO phiếu (ghi + cập nhật tồn kho) được thực hiện tại services/stock_service.py
để đảm bảo tính atomic (transaction) giữa nhiều bảng.
"""
...
class ImportReceiptDAO:
        ...
        sql = """
            SELECT r.*,
                   ...
                   (SELECT COALESCE(SUM(d.quantity * d.unit_price), 0)
                        FROM import_receipt_details d WHERE d.receipt_id = r.id) AS total_value
            FROM import_receipts r
        """
        conditions, params = [], []
        if keyword:
            conditions.append("(r.receipt_code LIKE ? OR r.supplier LIKE ? OR r.note LIKE ?)")
            kw = f"%{keyword}%"
    ...
    def get_by_id(self, receipt_id: int) -> dict | None:
        return db_manager.query_one("SELECT * FROM import_receipts WHERE id=?", (receipt_id,))
    ...
    def next_receipt_code(self) -> str:
        ...
        row = db_manager.query_one(
            "SELECT COUNT(*) c FROM import_receipts WHERE receipt_code LIKE ?",
            (f"{prefix}%",),
        )
        seq = row["c"] + 1
